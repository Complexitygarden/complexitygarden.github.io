...
function switchToDescriptionsView() {
    ...
    document.querySelector('.rightSidebarIconToggle').classList.add('disabled');
...
function switchToGraphView()
{
    ...
    document.querySelector('.rightSidebarIconToggle').classList.remove('disabled');
...
function setupClickableElements() {
    document.querySelectorAll('.clickable-class').forEach(el => {
        el.onclick = (e) => {
            ...
            if (className) {
                if (vis_type === 'descriptions') {
                    addDescriptionClassById(className);
                } else {
