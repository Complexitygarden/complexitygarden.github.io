...
function draw_graph(){
    ...
    d3.json('/get_complexity_network', function(data) {
        ...
        window.create_new_relationship_graph = function(){
            var firstNode = null;
            var tempLine = null;
            
            // Add a click handler to nodes that will be removed after first click
            var clickHandler = function(d) {
                // Prevent event propagation to stop side menu from opening
                d3.event.stopPropagation();
                
                if (!firstNode) {
                    // First click - store the node and create temporary line
                    firstNode = d;
                    console.log("First class selected:", d.name);
                    
                    // Create a temporary line that follows the mouse
                    tempLine = layer1.append("line")
                    ...
                    // Add mousemove handler to update the line - now on the visualization div
                    d3.select("#visualisation_div").on("mousemove", function() {
                        // Get the SVG element
                        var svg = d3.select("#visualisation_div svg").node();
                        // Get the current transform
                        var transform = d3.zoomTransform(svg);
                        // Get mouse position relative to the SVG
                        var mouse = d3.mouse(svg);
                        // Apply inverse transform to get coordinates in the SVG's coordinate system
                        var x = (mouse[0] - transform.x) / transform.k;
                        var y = (mouse[1] - transform.y) / transform.k;
                        
                        tempLine
                            .attr("x2", x)
                            .attr("y2", y);
                    });
                } else if (firstNode.name !== d.name) {
                    // Second click on a different node
                    console.log("Second class selected:", d.name);
                    
                    // Remove the temporary line and mousemove handler
                    if (tempLine) {
                        tempLine.remove();
                        tempLine = null;
                    }
                    d3.select("#visualisation_div").on("mousemove", null);
                    
                    // Call add_relationship with the two nodes
                    add_relationship(firstNode, d);
            ...
            // Add the click handler to all nodes
            nodeGroups.on("click", clickHandler);
        }
        
        function add_relationship(node_small, node_large){
            console.log("Adding relationship between " + node_small.name + " and " + node_large.name);
            
            // Make a fetch request to add the theorem
            fetch(`/add_theorem?theorem_type=containment&class_small=${node_small.name}&class_large=${node_large.name}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        console.log("Successfully added theorem");
                        // Redraw the graph to show the new relationship
                        draw_graph();
                    } else {
                        console.error("Failed to add theorem:", data.message);
                    }
                })
                .catch(error => {
                    console.error('Error adding theorem:', error);
                });
        }
